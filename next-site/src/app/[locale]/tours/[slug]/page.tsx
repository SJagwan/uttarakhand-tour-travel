  ...
  return (
    <article className="min-h-screen bg-white">
      ...
      <header className="relative w-full h-[60vh] md:h-[75vh] bg-slate-900 overflow-hidden">
        <Image src={tour.mainImage.url} alt={tour.mainImage.alt} fill priority className="object-cover opacity-70" sizes="100vw" />
      ...
      <div className="container mx-auto px-4 py-24">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-20 items-start">
          ...
          <div className="lg:col-span-2 space-y-24">
            ...
            {/* Real Photo Gallery */}
            {tour.gallery && <TourGallery items={tour.gallery} />}

            {/* Highlights */}
            <section className="bg-slate-50 rounded-[40px] p-12 border border-slate-100">
              <h2 className="text-3xl font-black text-slate-900 mb-10 tracking-tighter uppercase italic">Key Highlights</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {tour.highlights.map((h, i) => (
                  <div key={i} className="flex gap-4 items-start group">
                    ...
                    <span className="text-lg font-bold text-slate-700 tracking-tight">{h}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            {/* Itinerary */}
            <section>
              <h2 className="text-4xl font-black text-slate-900 mb-12 tracking-tighter uppercase italic">Detailed Itinerary</h2>
              <div className="space-y-8">
                {tour.itinerary.map((item, i) => (
                  <div key={i} className="relative pl-12 border-l-4 border-slate-100 pb-12 last:pb-0">
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-50 hover:shadow-xl transition-all duration-500 group">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        <span className="px-4 py-1.5 bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-xl italic">Day {item.day}</span>
                        <div className="flex gap-2">
                          {item.meals?.map(meal => (
                            <span key={meal} className="text-[10px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 px-3 py-1 rounded-lg"># {meal}</span>
                          ))}
                        </div>
                      </div>
                      ...
                      {item.accommodation && (
                        <div className="flex items-center gap-3 pt-6 border-t border-slate-100">
                          ...
                          <span className="text-xs font-black uppercase tracking-widest text-slate-400">Stay: <span className="text-slate-900">{item.accommodation}</span></span>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="p-12 bg-green-50 rounded-[40px] border border-green-100">
                <h3 className="text-2xl font-black text-green-900 mb-8 tracking-tighter uppercase italic flex items-center gap-3">
                  <CheckCircle2 size={24} className="text-green-600" /> Inclusions
                </h3>
                <ul className="space-y-5">
                  {tour.inclusions.map((item, i) => (
                    <li key={i} className="flex gap-3 text-sm font-bold text-green-800 tracking-tight leading-none italic">
              ...
              <div className="p-12 bg-slate-50 rounded-[40px] border border-slate-100">
                ...
                <ul className="space-y-5">
                  {tour.exclusions.map((item, i) => (
                    <li key={i} className="flex gap-3 text-sm font-bold text-slate-400 tracking-tight leading-none italic">
          ...
          <aside className="lg:sticky lg:top-32">
            <BookingWidget tourId={tour.id} tourTitle={tour.title} basePrice={tour.price} />
            <div className="mt-8 p-8 bg-slate-50 rounded-3xl border border-slate-100">
              <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4 italic">Cancellation Policy</h4>
              <p className="text-[10px] font-bold text-slate-400 leading-relaxed tracking-tight">
                * Cancellation before 30 days: 100% Refund <br />
                * Cancellation 15-30 days: 50% Refund <br />
